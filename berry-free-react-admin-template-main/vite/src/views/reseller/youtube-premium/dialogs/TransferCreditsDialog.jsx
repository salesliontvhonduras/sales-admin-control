...
import { colors, inputSx } from '../styles';
    ...
    <PremiumDialog
      ...
      actions={
        <Stack direction="row" spacing={1} sx={{ width: '100%', justifyContent: 'flex-end' }}>
          ...
          <Button variant="contained" disabled={saving || Number(credits || 0) <= 0} startIcon={saving ? <CircularProgress size={16} /> : null} onClick={() => onSubmit(Number(credits))}>
            Transferir
          </Button>
        </Stack>
      }
    >
      <Stack spacing={2}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5}>
          ...
          <Info label="Saldo esperado" value={`${formatCreditsFromUnits(currentUnits + transferUnits)} créditos`} />
        </Stack>
...
function Info({ label, value }) {
  return (
    <Stack sx={{ flex: 1, p: 1.25, bgcolor: colors.surface2, borderRadius: '8px', border: `1px solid ${colors.border}` }}>
